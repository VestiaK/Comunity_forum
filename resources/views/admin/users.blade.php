...
<div class="container mx-auto py-8">
    <h1 class="text-2xl font-bold mb-6 text-gray-900 dark:text-white">Manajemen User</h1>
                ...
                <tr class="border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <form method="POST" action="{{ route('admin.deleteUser', $user->id) }}">
                            @csrf
                            @method('DELETE')
                            <button class="bg-red-600 text-white px-2 py-1 rounded" onclick="return confirm('Yakin hapus user ini?')">Hapus</button>
                        </form>
                        @endif
                    </td>
                </tr>
    ...
    <div class="mt-4 flex justify-end items-center">
        <div>
            ...
            @else
                <a href="{{ $users->previousPageUrl() }}" class="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700">Previous</a>
            @endif
            <span class="mx-2 text-gray-900 dark:text-gray-200">Page {{ $users->currentPage() }} of {{ $users->lastPage() }}</span>
            @if($users->hasMorePages())
                <a href="{{ $users->nextPageUrl() }}" class="px-4 py-2 bg-purple-600 text-white rounded hover:bg-purple-700">Next</a>
            @else
